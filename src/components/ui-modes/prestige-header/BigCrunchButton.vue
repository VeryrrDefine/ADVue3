...
export default {
    ...
    computed: {
            ...
            const interFn = (index) => {
                ...
                if (ratio.lt(1)) {
                    ...
                    return Decimal.round(
                        new Decimal(1).sub(r).mul(stepRGB[0][index]).add(r.mul(stepRGB[1][index])),
                    )
                }
                if (ratio.lt(1.1)) {
                    const r = ratio.sub(1).mul(10)
                    return Decimal.round(
                        new Decimal(1).sub(r).mul(stepRGB[1][index]).add(r.mul(stepRGB[2][index])),
                    )
                }
        ...
        textInfo() {
            return i18n('inf', 'crunchFor', [
                `<span :style="amountStyle">${format(this.gainedIP, 2)}</span>
    ...
    methods: {
        update() {
            ...
            this.currentIPRate.copyFrom(
                gainedIP.div(Decimal.max(0.0005, Time.thisInfinityRealTime.totalMinutes)),
            )
    ...
    >
        ...
        <template v-else>
            ...
            <b>
                ...
                <span v-if="showIPRate">{{ i18n('inf', 'IPformats').split('$')[0] }}</span>
                <span v-else>{{
                    pluralize(i18n('inf', 'IPformats').split('$')[1], gainedIP)
                }}</span>
            </b>
            <template v-if="showIPRate">
                {{
                    i18n('inf', 'IPrate', [
                        format(currentIPRate, 2),
                        format(peakIPRate, 2),
                        format(peakIPRateVal, 2),
                    ])
                }}
            </template>
            <div v-else />
